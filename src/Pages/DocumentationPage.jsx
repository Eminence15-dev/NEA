  ...
  return (
    <div className={`min-h-screen ${bg} p-5`}>
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-[#050505] border border-[#b19149]/20 rounded-2xl shadow-xl p-10 mb-6 text-white">
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {TABS.map(t=>(
            <button key={t.key} onClick={()=>setDocsTab(t.key)}
              className={`px-5 py-2.5 rounded-xl font-semibold text-sm transition-all ${docsTab===t.key ? "bg-[#B83E18] text-black shadow-lg" : "bg-[#080808] text-[#f8d06b] hover:bg-[#1d1d1d] border border-[#b19149]/20"}`}>
              {t.label}
            </button>
          ))}
        </div>
        ...
        {docsTab === "algorithm" && (
          <div className={`${card} rounded-2xl shadow-xl p-8`}>
            ...
            <div className="p-4 bg-[#0B0B0B] border border-[#b19149]/20 rounded-xl border font-mono text-sm mb-6 overflow-auto">
            ...
            <div className="space-y-3 mb-6">
              {[
                ["Track Condition",              "text-[#D95A30]", "bg-[#0B0B0B] border-[#b19149]/20",      "Optimal = 1.000 · Good = 1.002 · Fair = 1.005 · Poor = 1.010","Polyurethane tracks rated optimal; worn/wet surfaces increase time."],
                ...
                <div key={factor} className={`p-4 rounded-xl border-2 ${styles}`}>
                  ...
                  <div className="font-mono text-xs rounded-lg px-3 py-2 mb-2 text-[#e3d38b] bg-[#111111]">{formula}</div>
                  <div className={`${muted} text-sm`}>{desc}</div>
                </div>
              ))}
            </div>
            <div className="p-5 rounded-xl border-2 mb-5 bg-[#0B0B0B] border-[#b19149]/20">
              <div className="font-bold text-sm mb-2 text-[#f8d06b]">🤖 XGBoost Machine Learning Correction</div>
              <div className="font-mono text-xs rounded-lg px-3 py-2 mb-2 text-[#e3d38b] bg-[#111111]">Final = physicsTime × 0.70 + xgboostTime × 0.30</div>
            ...
            <div className="p-4 rounded-xl border-2 text-sm bg-[#0B0B0B] border-[#b19149]/20 text-[#e3d38b]">
              <strong className="text-[#f8d06b]">Accuracy note:</strong> ~70% accuracy (±0.20s) for standard conditions. Actual performance is also influenced by race strategy, reaction time, and biomechanics.
            </div>
          </div>
        )}
