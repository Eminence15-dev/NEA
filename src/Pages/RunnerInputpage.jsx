...
const RunnerInputPage = ({
  ...
  const inputBase  = "bg-[#0f0f0f] text-[#f8d06b] placeholder-[#8f7d45] border-[#b19149]/20";
      ...
      <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-2">
            {[["100","100m","bg-[#B83E18] hover:bg-[#8F2E0E]"],["200","200m","bg-[#1A3FA0] hover:bg-[#162F7A]"]].map(([val,label,col]) => (
              <button key={val} type="button"
                ...
                className={`p-3 rounded-lg font-semibold transition-all text-sm ${formData.eventDistance===val ? "bg-[#b19149] text-black shadow-lg" : "bg-[#0f0f0f] text-[#f8d06b] border border-[#b19149]/20 hover:bg-[#1d1d1d]"}`}>
                {label}
              </button>
            ))}
          </div>
        ...
        <div className="relative">
          ...
          <input type="text" name="athleteName" value={formData.athleteName} autoComplete="off"
            ...
            className={`w-full p-3 rounded-lg outline-none transition-all ${inputBase}
              ${!touched.athleteName ? "border-[#b19149]/20 focus:border-[#f8d06b]" : selectedAthlete ? "border-[#f8d06b] bg-[#1a1a1a]" : "border-[#b19149]/30 bg-[#2f1a0c]/20"}`}/>
          ...
          {dropdownOpen && suggestions.length > 0 && (
            <div className="absolute z-20 w-full mt-1 bg-[#0b0b0b] border border-[#b19149]/20 rounded-xl shadow-xl max-h-64 overflow-y-auto">
              <div className="px-3 py-2 bg-[#090909] border-b border-[#b19149]/10 text-xs text-[#a78b3c] font-semibold">
              ...
              {suggestions.map((athlete,i) => (
                <button key={i} type="button" onMouseDown={()=>selectAthlete(athlete)}
                  className="w-full text-left px-4 py-3 bg-[#0b0b0b] hover:bg-[#1d1d1d] transition-colors flex items-center justify-between border-b border-[#b19149]/10 last:border-0">
                  <div>
                    <div className="font-semibold text-[#f8d06b] text-sm flex items-center gap-2">
                      {athlete.name}
                      {athlete.custom && <span className="text-xs px-1.5 py-0.5 rounded-full bg-[#b19149]/20 text-[#f8d06b]">Custom</span>}
                      {athlete.gender && <span className={`text-xs px-1.5 py-0.5 rounded-full font-semibold bg-[#b19149]/20 text-[#f8d06b]`}>{athlete.gender==="male"?"👨 Male":"👩 Female"}</span>}
                    </div>
                    <div className="text-xs text-[#a78b3c]">{athlete.country} · {athlete.raceTime}s · {athlete.city} ({athlete.year})</div>
                  </div>
                  <span className="text-xs px-2 py-0.5 rounded-full font-semibold bg-[#b19149]/20 text-[#f8d06b]">{athlete.event}m</span>
                </button>
              ))}
            </div>
          )}
        ...
        {/* Original Race Info */}
        {selectedAthlete && (
          <div className="p-4 rounded-xl border-2 bg-[#0f0f0f] border-[#b19149]/20">
            <div className="text-xs font-semibold text-[#f8d06b] mb-2 uppercase tracking-wide">📍 Original Race Conditions (pre-filled below)</div>
        ...
        {/* Track Condition */}
        <div className="p-4 rounded-lg border-2 bg-[#0f0f0f] border-[#b19149]/20">
        ...
        {/* Warnings */}
        {warnings.length > 0 && (
          <div className="p-4 rounded-lg border-2 space-y-2 bg-[#2f1200] border-[#b19149]/20">
            <div className="flex items-center gap-2 font-semibold text-sm text-[#f8d06b]"><AlertTriangle size={16}/>Condition Warnings</div>
            {warnings.map((w,i)=><div key={i} className="flex items-start gap-2 text-xs text-[#a78b3c]"><AlertTriangle size={12} className="mt-0.5 shrink-0"/>{w}</div>)}
          </div>
        )}
        ...
        <button onClick={handleSubmit} disabled={!formIsValid}
          className={`w-full p-4 rounded-xl text-lg font-bold transition-all flex items-center justify-center gap-2
            ${formIsValid ? "bg-[#b19149] hover:bg-[#f8d06b] text-black hover:shadow-2xl cursor-pointer" : "bg-[#1a1a1a] text-[#6b5a25] cursor-not-allowed"}`}>
